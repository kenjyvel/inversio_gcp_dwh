...
def bq_job_dwh_f_despacho_productos(table_id,bq_client):
    ...
    bq_query = (
                ...
                IF NOT EXISTS (SELECT 1 FROM dwh.__TABLES__ WHERE table_id='f_despacho_productos')
                    THEN 
                        CREATE TABLE dwh.f_despacho_productos AS
                            SELECT * FROM stage.f_despacho_productos;
                    ELSE 
                        BEGIN
                        ...
                        table1_columns AS ( --Aqui se obtiene la lista de columnas de stage
                            SELECT column FROM (SELECT * FROM `stage.f_despacho_productos` LIMIT 1) t,
                            UNNEST(REGEXP_EXTRACT_ALL(TRIM(TO_JSON_STRING(t), '{}'), r'"([^"]*)":')) column
                            ), 
                        table2_columns AS ( --Aqui se obtiene la lista de columnas de dwh
                            SELECT column FROM (SELECT * FROM `dwh.f_despacho_productos` LIMIT 1) t,
                            UNNEST(REGEXP_EXTRACT_ALL(TRIM(TO_JSON_STRING(t), '{}'), r'"([^"]*)":')) column
                            ), 
                        ...
                        SELECT (
                            ...
                            SELECT 
                                ...
                                ' FROM `dwh.f_despacho_productos` d WHERE NOT EXISTS(SELECT pkPunto FROM stage.f_despacho_productos s WHERE d.pkPunto=s.pkPunto)'
                            FROM print_table p
                            ) 
                        );
