...
def bq_job_dwh_f_abastecimiento(table_id,bq_client):
    ...
    bq_query = (
                ...
                IF NOT EXISTS (SELECT 1 FROM dwh.__TABLES__ WHERE table_id='f_abastecimiento')
                    THEN 
                        CREATE TABLE dwh.f_abastecimiento AS
                            SELECT * FROM stage.f_abastecimiento;
                    ELSE 
                        BEGIN
                        ...
                        table1_columns AS ( --Aqui se obtiene la lista de columnas de stage
                            SELECT column FROM (SELECT * FROM `stage.f_abastecimiento` LIMIT 1) t,
                            UNNEST(REGEXP_EXTRACT_ALL(TRIM(TO_JSON_STRING(t), '{}'), r'"([^"]*)":')) column
                            ), 
                        table2_columns AS ( --Aqui se obtiene la lista de columnas de dwh
                            SELECT column FROM (SELECT * FROM `dwh.f_abastecimiento` LIMIT 1) t,
                            UNNEST(REGEXP_EXTRACT_ALL(TRIM(TO_JSON_STRING(t), '{}'), r'"([^"]*)":')) column
                            ), 
                        ...
                        SELECT (
                            ...
                            SELECT 
                                ...
                                ' FROM `dwh.f_abastecimiento` d WHERE NOT EXISTS(SELECT pkTiempo FROM stage.f_abastecimiento s WHERE d.pkTiempo=s.pkTiempo)'
                            FROM print_table p
                            ) 
                        );
