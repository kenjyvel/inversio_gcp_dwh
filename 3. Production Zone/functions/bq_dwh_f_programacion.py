...
def bq_job_dwh_f_programacion(table_id,bq_client):
    ...
    bq_query = (
                ...
                IF NOT EXISTS (SELECT 1 FROM dwh.__TABLES__ WHERE table_id='f_programacion')
                    THEN 
                        CREATE TABLE dwh.f_programacion AS
                            SELECT * FROM stage.f_programacion;
                    ELSE 
                        BEGIN
                        ...
                        table1_columns AS ( --Aqui se obtiene la lista de columnas de stage
                            SELECT column FROM (SELECT * FROM `stage.f_programacion` LIMIT 1) t,
                            UNNEST(REGEXP_EXTRACT_ALL(TRIM(TO_JSON_STRING(t), '{}'), r'"([^"]*)":')) column
                            ), 
                        table2_columns AS ( --Aqui se obtiene la lista de columnas de dwh
                            SELECT column FROM (SELECT * FROM `dwh.f_programacion` LIMIT 1) t,
                            UNNEST(REGEXP_EXTRACT_ALL(TRIM(TO_JSON_STRING(t), '{}'), r'"([^"]*)":')) column
                            ), 
                        ...
                        SELECT (
                            ...
                            SELECT 
                                ...
                                ' FROM `dwh.f_programacion` d WHERE NOT EXISTS(SELECT pkTiempo FROM stage.f_programacion s WHERE d.pkTiempo=s.pkTiempo)'
                            FROM print_table p
                            ) 
                        );
