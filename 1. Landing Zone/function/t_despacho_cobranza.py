...
def t_despacho_cobranza_to_bq(mysql_conn,table_id,bq_client):
    
    conn=mysql_conn
    client=bq_client
      
    today = date.today()
    d1 = today-timedelta(days = 250)
    ...
    query = f"""SELECT
                ...
                tipoDoc,
                pagado,
                observDetallePorCobrar,
                usuario,
                fchCreacion,
                usuarioCreaCobranza,
                fchCreaCobranza
                FROM despachodetallesporcobrar where fchDespacho >= '{d1}' """
