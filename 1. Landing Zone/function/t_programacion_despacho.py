...
def t_programacion_despacho_to_bq(mysql_conn,table_id,bq_client):
    ...
    query = f"""SELECT
                ...
                editaUsuario,
                editaFch
                FROM programdespacho where fchDespacho >= '{d1}' """
                
    df=pd.read_sql_query(query, conn)

    df["hraInicioEsperada"]=df["hraInicioEsperada"].dt.total_seconds().round().astype(str)
    ...
    job_config = bigquery.LoadJobConfig(
        schema=[
            ...
            bigquery.SchemaField("hraInicioEsperada", bigquery.enums.SqlTypeNames.STRING),
            bigquery.SchemaField("creacFch", bigquery.enums.SqlTypeNames.DATE),
