...
def t_doc_cobranza_to_bq(mysql_conn,table_id,bq_client):
    ...
    query = f"""SELECT
                ...
                editUsuario,
                editFch
                FROM doccobranza where doccobranza >= '{d1}' """
                
    df=pd.read_sql_query(query, conn)

    df["hraCreacion"]=df["hraCreacion"].dt.total_seconds().round().astype(str)
    df["fchCarga"]=datetime_load


    job_config = bigquery.LoadJobConfig(
        schema=[
            bigquery.SchemaField("fchConfirmacion", bigquery.enums.SqlTypeNames.DATE),
        ],
