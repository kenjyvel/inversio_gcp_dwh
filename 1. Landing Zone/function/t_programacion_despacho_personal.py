...
def t_programacion_personal_to_bq(mysql_conn,table_id,bq_client):
    ...
    query = f"""SELECT
                ...
                ultProcesoFch,
                ultProcesoUsuario
                FROM programdesppers 
                WHERE idProgram IN (SELECT distinct idProgramacion from despacho where fchDespacho >= '{d1}')
                and idProgram<>0"""
